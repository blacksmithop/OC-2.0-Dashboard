...
export default function CrimeAnalysis({ crimes, members }: CrimeAnalysisProps) {
  ...
  const stats = useMemo(() => {
    ...
    const nearFull = analyzedCrimes.filter(
      (c) => c.isNearFull && !c.isFull,
    ).length;
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-3">
        ...
        <div>
          <label className="text-sm text-foreground font-semibold block mb-2">
            Sort by:
          </label>
          <select
            value={sortBy}
            onChange={(e) =>
              setSortBy(e.target.value as "fill" | "difficulty" | "name")
            }
              ...
              <div className="mb-2">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-primary">
                    {Math.round(crime.fillPercentage)}%
                  </span>
                </div>
              ...
              <div className="text-xs">
                ...
                <div className="flex flex-wrap gap-1">
                  {crime.slots.map((slot) => (
                    <span
                      key={slot.position_info?.id || slot.position_id}
                    ...
                    >
                      {slot.position}:{" "}
                      {slot.user ? memberMap.get(slot.user.id) : "Empty"}
                    </span>
                  ))}
                </div>
              </div>
